...
import pandas as pd 
import math
data= pd.read_csv('vivanta1.csv')
#data= pd.read_csv('brunton.csv')
#data= pd.read_csv('fresco.csv')
data= data.replace(0.0, np.NaN) 
names_data = pd.read_csv('hotelnames.csv')
my_hotel_id=1				#vivanta
#my_hotel_id=1084    		#brunton
#my_hotel_id= 15627
percent_value= 0.30                   #defining threshold for abnormal pricing
hotel_id = [ 1,2,3,4,5]		#vivanta
#hotel_id=[1084,612,613,1085,1086,1087]	#brunton
#hotel_id=[15627,15628,15629,15630,15631,15632,15633,15634] #fresco 
...
for i in range(n):
    ...
    min_price[i]= hotel[i].groupby(['check_in'], sort=True)['total'].min()
   		#calculating the minimum price of each day of all hotels
